...
from services.extraction.lsp.protocol import ServerExited
...
class LanguageServerManager:
    def __init__(
        ...
        *,
        ...
        # Cancellation binding; None (the default) means requests block as they always did.
        self._abort = None
        self._stage = "setup"
    ...
    def client_for(self, path: Path) -> LspClient | None:
        ...
        with self._lock:
            ...
            if self._abort is not None:
                # Lazily created: a client that appears mid-cancel must be interruptible
                # from its first request, not from the next bind_run.
                client.bind_abort(self._abort, stage=self._stage)
    ...
    def bind_run(self, abort=None, *, stage: str = "setup") -> None:
        """Make every request from now on interruptible, including from current clients.

        Clients are created lazily per language, so the binding is stored and applied to
        each new one as it appears; existing clients are rebound immediately. Called by the
        pipeline when it registers this manager with the worker's teardown handle.
        """
        with self._lock:
            self._abort = abort
            self._stage = stage
            clients = list(self._clients.values())
        for client in clients:
            client.bind_abort(abort, stage=stage)

    def kill_all(self) -> None:
        """Terminate every server without the parting handshake. The cancel path."""
        with self._lock:
            clients = list(self._clients.values())
            self._clients.clear()
        for client in clients:
            try:
                ...
                log.debug("failed to kill LSP client", exc_info=True)
