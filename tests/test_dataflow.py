...
from aegis_core.workspace import (
    dominant_suffix,
    iter_source_files,
    suffix_census,
    workspace_digest,
...
def test_the_sink_report_carries_the_enclosing_methods_parameters() -> None:
    """The same query that finds the method reports what it takes, so `auto` needs no second trip.

    The receiver is included and filtered later: which parameter is the receiver is a naming
    convention (`this` / `self` / `cls`), and the report is the wrong layer to hold an opinion.
    """
    text = "\n".join([
        "METHOD\tquery_user",
        "PARAM\tself",
    ...
    assert resolution.parameters == ["self", "user_id"]


def test_an_auto_anchor_uses_the_hit_methods_own_parameter() -> None:
    """`auto` with no hint: the enclosing method and its first non-receiver parameter.

    A receiver anchor returns a path that proves nothing, so it is skipped. When the method has
    only a receiver there is nothing to anchor on -- `("", "")` -- and the configured default takes
    over, the same last resort as before.
    """
    ...
    resolution = SinkResolution(anchor="execute", method="query_user", parameters=["self", "user_id"])
    assert _auto_source(SourceAnchor(kind=ANCHOR_AUTO), resolution) == ("query_user", "user_id")
    # A hint means "ask about this instead", so the enclosing method is not used.
    assert _auto_source(SourceAnchor(kind=ANCHOR_AUTO, needle="os.getenv"), resolution) == ("", "")
    # Fixed kinds already carry their own method and pass nothing through.
    assert _auto_source(SourceAnchor(kind=ANCHOR_PARAMETER, method="m"), resolution) == ("", "")
    # Only a receiver: nothing to anchor on.
    only_receiver = SinkResolution(anchor="execute", method="run", parameters=["self"])
    assert _auto_source(SourceAnchor(kind=ANCHOR_AUTO), only_receiver) == ("", "")


def test_the_auto_scala_expression_carries_the_hint_the_method_and_the_fallback() -> None:
    """Everything `auto` needs is substituted into one expression, in the existing round trip."""
    from services.dataflow.worker import SourceAnchor

    expression = SourceAnchor(kind="auto", needle='os.getenv("X")').scala(
        auto_method="handler", auto_param="request", fallback="request.args.get"
    )

    assert expression.startswith('anchoredAuto("os.getenv(\\"X\\")"')
    assert '"handler"' in expression and '"request"' in expression
    assert '"request.args.get"' in expression
...
def _frontend_worker(tmp_path: Path, *, args: dict | None = None):
    """A worker whose `frontends_dir` is a local stand-in, so no Joern install is needed."""
    from aegis_core.config import DataflowConfig
    from services.dataflow.worker import JoernWorker

    frontends = tmp_path / "frontends"
    for name in ("pysrc2cpg", "javasrc2cpg"):
        (frontends / name / "bin").mkdir(parents=True, exist_ok=True)
        (frontends / name / "bin" / name).write_text("#!/bin/sh\n", encoding="utf-8")
    config = DataflowConfig(
        cache_dir=tmp_path / "cache",
        frontends_dir=frontends,
        cpg_frontend_args=args or {},
    )
    return JoernWorker(config, workspace_root=tmp_path), frontends
...
def test_the_cpg_cache_key_carries_the_frontend(tmp_path: Path) -> None:
    """One workspace has a different graph per frontend, so the key has to say which one built it.

    `shared_dir` is shared by the whole worker fleet and the key used to be a function of the
    workspace content alone -- so the moment a second frontend exists (Java needs `javasrc2cpg`,
    Python uses `pysrc2cpg`) a project would have been answered from the graph the *other* one
    built, out of a cache both workers read. Same class of bug `workspace_digest` records about
    hashing only `.py` files: two unrelated Java projects collided on `sha1("")` and the second was
    answered from the first one's graph.
    """
    worker, _frontends = _frontend_worker(tmp_path)
    python_repo = _repo(tmp_path, "py", {"a.py": "def f():\n    return 1\n"})
    java_repo = _repo(tmp_path, "java", {"A.java": "class A {}\n"})

    assert worker.frontend_for(python_repo).name == "pysrc2cpg"
    assert worker.frontend_for(python_repo).suffix == ".py"
    assert worker.frontend_for(java_repo).name == "javasrc2cpg"

    py_key = worker.graph_key("deadbeef", worker.frontend_for(python_repo))
    java_key = worker.graph_key("deadbeef", worker.frontend_for(java_repo))
    assert py_key == "deadbeef-pysrc2cpg"
    assert py_key != java_key, "one frontend must never be answered from another frontend's graph"
    assert worker._cpg_path("deadbeef", worker.frontend_for(python_repo)) == (
        worker.shared_dir / "deadbeef-pysrc2cpg" / "cpg.bin"
    )

    # `/health`'s `loaded` is the project, not the cache key.
    worker._loaded = ("deadbeef", "pysrc2cpg")
    assert worker.loaded_project == "deadbeef"


def test_a_language_without_a_frontend_is_refused_by_name(tmp_path: Path) -> None:
    """No frontend for this tree is a refusal, not an empty graph.

    An empty graph is indistinguishable from a clean codebase at every layer above the worker:
    every question comes back "the anchor matches no node", which validation reads as evidence
    about the code. The census travels in the message because the fix is an operator decision.
    """
    from services.dataflow.worker import NoFrontend

    worker, _frontends = _frontend_worker(tmp_path)
    go_repo = _repo(tmp_path, "go", {"main.go": "package main\n"})

    with pytest.raises(NoFrontend) as excinfo:
        worker.frontend_for(go_repo)

    message = str(excinfo.value)
    assert "main.go" not in message  # names the census, not every file
    assert ".py" in message and ".java" in message, "the configured suffixes must be named"
    assert "empty graph" in message, "and why refusing beats building one"


def test_a_missing_frontend_binary_refuses_instead_of_building_an_empty_graph(
    tmp_path: Path,
) -> None:
    """Configured but not installed is its own failure, and it is per workspace.

    Deliberately not a startup check: a deployment that has configured a language this image
    cannot build must still be able to analyse the languages it can.
    """
    from services.dataflow.worker import JoernUnavailable

    worker, frontends = _frontend_worker(tmp_path)
    java_repo = _repo(tmp_path, "java", {"A.java": "class A {}\n"})
    (frontends / "javasrc2cpg" / "bin" / "javasrc2cpg").unlink()

    # Python still works...
    python_repo = _repo(tmp_path, "py", {"a.py": "x = 1\n"})
    assert worker.frontend_for(python_repo).name == "pysrc2cpg"

    # ...and Java says exactly what is wrong.
    with pytest.raises(JoernUnavailable) as excinfo:
        worker.frontend_for(java_repo)
    assert "javasrc2cpg" in str(excinfo.value)
    assert "not installed" in str(excinfo.value)


def test_a_java_classpath_reaches_the_frontend_argv(tmp_path: Path) -> None:
    """`javasrc2cpg` needs a classpath, and it must arrive as separate argv words.

    Without one it cannot resolve framework types, so `HttpServletRequest.getParameter` is an
    unknown call and the source anchor matches no node on any real Spring project -- the whole
    question is wasted. A list rather than a string because a path containing a space must not be
    split.
    """
    args = {".java": ["--classpath", "/deps/a b.jar"]}
    worker, _frontends = _frontend_worker(tmp_path, args=args)
    java_repo = _repo(tmp_path, "java", {"A.java": "class A {}\n"})

    frontend = worker.frontend_for(java_repo)
    assert frontend.extra_args == ("--classpath", "/deps/a b.jar")
    assert worker.frontend_for(_repo(tmp_path, "py", {"a.py": "x = 1\n"})).extra_args == ()


def test_the_suffix_census_picks_the_dominant_language(tmp_path: Path) -> None:
    """Which frontend builds the graph is decided by counting files, and the count is stable.

    Ties break on the suffix in sorted order: the answer also decides the CPG cache key, and an
    unstable key is a rebuild on every request.
    """
    root = _repo(tmp_path, "mixed", {"a.py": "", "b.py": "", "A.java": ""})
    assert dominant_suffix(suffix_census(root, {".py", ".java"})) == ".py"

    tied = _repo(tmp_path, "tied", {"a.py": "", "A.java": ""})
    assert dominant_suffix(suffix_census(tied, {".py", ".java"})) == ".java"

    assert dominant_suffix(suffix_census(tied, {".go"})) is None
    assert dominant_suffix({}) is None
...
def test_an_engine_that_proves_no_path_records_a_prune_and_skips_the_crawl(
    ...
    ws, _resolver, focus = _focus(workspace)
    # Both anchors have to have *landed* on the graph for zero flows to be a statement about the
    # code. A bundle whose source matched no node is a statement about the question instead, and
    # falls back to the crawl -- see the `..._unmatched` tests below. The defaults are 0, so this
    # fixture used to pass while describing the wrong situation.
    stub = StubDataflow(
        FlowBundle(source_anchor="a", sink_anchor="b", source_candidates=1, sink_candidates=1)
    )
...
def test_an_unresolved_sink_is_named_differently_from_a_missing_path(workspace: Path) -> None:
    ...
    assert "dataflow_no_path" not in rules
    # And it falls back to the crawl. "We did not know what to look for" is not a reason to throw
    # away the call graph that was already assembled -- measured on a Java project whose CPG was
    # built by the wrong frontend, where every question ended up here and the slice shrank to the
    # focus method alone.
    assert len(slice_.methods) > 1, "an unresolved sink must fall back to the crawl"
    assert slice_.edges, "the crawl is what puts edges on the slice"


def test_a_source_anchor_that_matched_nothing_is_not_reported_as_no_path(
    workspace: Path,
) -> None:
    """`source_candidates == 0` says the question never landed, not that the code is clean.

    Reported as `dataflow_no_path`, it claims "the value cannot reach the sink" about code the
    engine never read -- and the single-method slice it returns silently replaces the call graph
    the language server had already produced.
    """
    ws, _resolver, focus = _focus(workspace)
    bundle = FlowBundle(
        source_anchor="request.args.get",
    ...
    rules = [prune.rule for prune in slice_.prunes]
    assert "dataflow_source_unmatched" in rules
    assert "dataflow_no_path" not in rules
    assert len(slice_.methods) > 1, "the crawl must run when the question never landed"


def test_a_sink_anchor_that_matched_no_node_is_not_reported_as_no_path(workspace: Path) -> None:
    """A derived sink name that matches no call is the same class of failure as no sink at all."""
    ws, _resolver, focus = _focus(workspace)
    bundle = FlowBundle(
        source_anchor="request.args.get",
        sink_anchor="execute",
    ...
    rules = [prune.rule for prune in slice_.prunes]
    assert "dataflow_sink_unmatched" in rules
    assert "dataflow_no_path" not in rules
    assert len(slice_.methods) > 1, "the crawl must run when the question never landed"
...
def test_a_resolved_sink_with_no_path_is_reported_as_no_path(workspace: Path) -> None:
    ...
    assert "dataflow_sink_unmatched" not in rules
    assert "dataflow_source_unmatched" not in rules
