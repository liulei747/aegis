...
def test_worker_verifier_sends_the_finding_position_and_never_a_sink(repo: Path) -> None:
    ...
    assert call["source_kind"] == "call"


def test_worker_verifier_uses_a_parameter_anchor_for_a_bare_function_hint(repo: Path) -> None:
    """A method *name* as a call anchor resolves to the call site and finds nothing."""
    verifier, client, _ = _verifier(repo)
    verifier.verify(file="handler.py", line=2, source_hint="handle_request")
    call = client.calls[0]
    assert call["source_kind"] == "parameter"
    assert call["source_method"] == "handle_request"
    assert call["source_parameter"] == ""


def test_worker_verifier_uses_a_call_anchor_when_the_hint_is_not_a_local_function(
    repo: Path,
) -> None:
    verifier, client, factory = _verifier(repo)
    verifier.verify(file="app.py", line=6, source_hint="getenv")
    assert client.calls[0]["source_kind"] == "call"
    # The call needle travels inside the client's config; there is no per-call argument for it.
    assert factory.configs[0].source == "getenv"


def test_worker_verifier_derives_the_enclosing_function_when_there_is_no_hint(repo: Path) -> None:
    verifier, client, _ = _verifier(repo)
    verifier.verify(file="pkg/mod.py", line=2, source_hint=None)
    call = client.calls[0]
    assert call["source_kind"] == "parameter"
    assert call["source_method"] == "handler"
    assert call["source_parameter"] == "request"


def test_worker_verifier_falls_back_to_the_configured_anchor_without_a_function(repo: Path) -> None:
    verifier, client, factory = _verifier(repo)
    verifier.verify(file="app.py", line=3, source_hint=None)
    call = client.calls[0]
    assert call["source_kind"] == "call"
    assert call["source_method"] == ""
    assert factory.configs[0].source == "request.args.get"
